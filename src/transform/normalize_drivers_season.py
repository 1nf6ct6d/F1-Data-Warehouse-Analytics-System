...
def get_latest_drivers_raw_file_for_season(season: int) -> Path:
    files = sorted(RAW_DIR.glob(f"drivers_{season}_*.json"))
    if not files:
        raise FileNotFoundError(f"В data/raw не найдено ни одного raw-файла для drivers сезона {season}")
    return files[-1]
...
def normalize_drivers_for_season(season: int) -> None:
    raw_file = get_latest_drivers_raw_file_for_season(season)
    ...
    if not rows:
        raise ValueError(f"Нет данных drivers для сезона {season}")

    STAGING_DIR.mkdir(parents=True, exist_ok=True)
    output_path = STAGING_DIR / f"stg_drivers_{season}.csv"
    ...
    print(f"[STAGING] drivers season={season}, rows={len(rows)}")