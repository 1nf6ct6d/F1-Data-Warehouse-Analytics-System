...
def build_fact_rows(connection) -> list[tuple]:
    file_path = STAGING_DIR / "stg_race_results.csv"
...
def load_fact_race_result(connection) -> None:
    rows = build_fact_rows(connection)
    ...
    print(f"[DWH] Загружено race results: {len(rows)}")


def main() -> None:
    connection = get_oracle_connection()
    load_fact_race_result(connection)
    connection.close()
    print("[DWH] Загрузка fact_race_result завершена")
